...
def savesample(fname,m_rec,σ2_rec,meandata,lon,lat,time,e,ii,offset,
    ...
    if ii == 0 and offset == 0:  # Create file only on the very first batch
        # create file
        ...
        nc_time.units = "days since 1900-01-01 00:00:00"
        ...
        # --- GET TIME VARIABLE TO APPEND ---
        nc_time = root_grp.variables['time']

    # --- WRITE TIME DATA FOR THE CURRENT BATCH ---
    time_units = "days since 1900-01-01 00:00:00"
    time_calendar = "gregorian"
    batch_times = time[offset:offset + m_rec.shape[0]]
    if len(batch_times) > 0:
        nc_time[offset:offset + m_rec.shape[0]] = date2num(batch_times, units=time_units, calendar=time_calendar)
...
def reconstruct(lon, lat, time, mask, meandata,
...
"""
    ...
    train_dataset = tf.data.Dataset.from_generator(
        ...
        (tf.TensorShape([jmax,imax,nvar]),tf.TensorShape([jmax,imax,2]))).repeat().shuffle(shuffle_buffer_size).batch(batch_size).prefetch(tf.data.AUTOTUNE)
    ...
    test_iterator = tf.compat.v1.data.Iterator.from_structure(tf.compat.v1.data.get_output_types(test_dataset),
    ...
    for l in range(1,enc_nlayers):
        enc_conv[l] = tf.keras.layers.Conv2D(filters=enc_nfilter[l],
                                   kernel_size=(3,3),
                                   padding='same',
                                   activation=conv_activation)(enc_avgpool[l-1])
        print("encoder: output size of convolutional layer: ",l,enc_conv[l].shape)

        enc_avgpool[l] = tf.keras.layers.AveragePooling2D(pool_size=(2,2),
                                                strides=(2,2),
    ...
    if len(frac_dense_layer) == 0:
        ...
        for i in range(2*len(frac_dense_layer)):
            dense[2*i+1] = tf.keras.layers.Dense(units=dense_units[i],
                                           activation=tf.nn.relu)(dense[2*i])
            print("dense layer: output units: ",i,dense[2*i+1].shape)
            dense[2*i+2] = tf.keras.layers.Dropout(rate=dropout_rate_train)(dense[2*i+1])
    ...
    for l in range(1,enc_nlayers):
        ...
        dec_conv[l] = tf.keras.layers.Conv2D(
    ...
    loginvσ2_rec = tf.clip_by_value(loginvσ2_rec, -10., 10.) # Clip to a safe range

    invσ2_rec = tf.exp(loginvσ2_rec)
    ...
    if truth_uncertain:
        ...
        cost = (tf.reduce_sum(input_tensor=tf.multiply(
            tf.math.log((σ2_rec + epsilon)/(σ2_true + epsilon)) + (σ2_true + difference**2) / (σ2_rec + epsilon),mask_noncloud))) / (n_noncloud + epsilon)
    else:
        cost = (tf.reduce_sum(input_tensor=tf.multiply(tf.math.log(σ2_rec + epsilon),mask_noncloud)) +
            tf.reduce_sum(input_tensor=tf.multiply(difference**2 / (σ2_rec + epsilon),mask_noncloud))) / (n_noncloud + epsilon)
    ...
    if load_model is not None:
        print(f"Restoring model from :{load_model}")
        saver.restore(sess,load_model)
        sess.run(tf.compat.v1.variables_initializer(optimizer.variables()))
        try:
            base_name= os.path.basename(load_model)
            start_epoch= int(base_name.split('-')[1].split('.')[0])
            print(f"Model restored , Resuming training from {start_epoch}")
        except (IndexError,ValueError):
            print("could not determine resume epoch from filename , Starting from epoch 0 ")
            start_epoch = 0
    ...
    for e in range(start_epoch,epochs):
        ...
        if ((e == epochs-1) or ((save_each > 0) and (e % save_each == 0))) and outdir != None:
            ...
            for ii in range(ceil(test_len / batch_size)):
                ...
                savesample(fname,batch_m_rec,batch_σ2_rec,meandata,lon,lat,time, e,ii,
...
def reconstruct_from_checkpoint(
    lon, lat,time, mask, meandata,
    inference_datagen, inference_len,
    ...
    """
    Reconstructs data using a pre-trained model from a checkpoint.
    This function is for inference only.
    """
    ...
    print(f"  - nvar: {nvar}")
    print(f"  - ntime_win: {ntime_win}")
    print(f"  - enc_nfilter_internal: {enc_nfilter_internal}")
    print(f"  - skipconnections: {skipconnections}")
    print("-" * 25)
    ...
    inference_dataset = tf.data.Dataset.from_generator(
        inference_datagen, (tf.float32, tf.float32),
        (tf.TensorShape([jmax, imax, nvar]), tf.TensorShape([jmax, imax, 2]))).batch(batch_size)
    
    iterator = tf.compat.v1.data.make_one_shot_iterator(inference_dataset)
    inputs_, xtrue = iterator.get_next()
    ...
    enc_avgpool[0] = inputs_
    for l in range(1, enc_nlayers):
        enc_conv[l] = tf.keras.layers.Conv2D(filters=enc_nfilter[l], kernel_size=(3, 3), padding='same', activation=conv_activation)(enc_avgpool[l-1])
    ...
    if len(frac_dense_layer) > 0:
        ...
        #loop
        dense_input = avgpool_flat
        for i, units in enumerate(dense_units):
            dense_input = tf.keras.layers.Dense(units=units, activation=tf.nn.relu)(dense_input)
        dense_2d = tf.reshape(dense_input, tf.shape(input=enc_last))
    ...
    for l in range(1, enc_nlayers):
        l2 = enc_nlayers - l
        dec_upsample[l] = tf.image.resize(dec_conv[l-1], enc_conv[l2].shape[1:3], method=resize_method)
        if l in skipconnections:
            dec_upsample[l] = tf.concat([dec_upsample[l], enc_avgpool[l2 - 1]], 3)
        dec_conv[l] = tf.keras.layers.Conv2D(filters=dec_nfilter[l], kernel_size=(3, 3), padding='same', activation=conv_activation)(dec_upsample[l])
    ...
    saver = tf.compat.v1.train.Saver(tf.compat.v1.trainable_variables())
    print(f"Restoring model from: {load_model_path}")
    saver.restore(sess, load_model_path)
    print("Model restored successfully.")

    fname = os.path.join(outdir, output_filename)
    print(f"Starting reconstruction. Output will be saved to {fname}")

    dt_start = datetime.now()
    
    for ii in range(ceil(inference_len / batch_size)):
        print(f"Processing batch {ii + 1}/{ceil(inference_len / batch_size)}...")
        try:
            batch_m_rec, batch_σ2_rec = sess.run([m_rec, σ2_rec])
            offset = ii * batch_size
            savesample(fname, batch_m_rec, batch_σ2_rec, meandata, lon, lat,time, 0, ii,
                       offset, transfun=transfun)
    ...
    print(f"Total time: {dt_end - dt_start}")
